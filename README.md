...
├──App.java      # Clase principal con la logica de inicio de la aplicacion
├── ui/          # paquete que contiene la interfaz grafica del sistema
├── data/        # paquete que contiene la logica de datos (GestorEntidades, etc.)
├── model/       # paquete que contiene la logica de dominio (UnidadOperativa, PlantaProceso, CentroCultivo, etc.)
├── utils/       # paquete que contiene las utilidades y validaciones
📁 src/main/resources/
├── Empleados.csv  # paquete que contiene la informacion de los empleados de salmontt
├── UnidadesOperativas.csv  # paquete que contiene la informacion de las unidades operativas de salmontt
```
...
3. Verifica que los archivos `.csv` estén correctamente ubicados.

4. Ejecuta el archivo `App.java` desde el paquete `cl.salmontt`.
